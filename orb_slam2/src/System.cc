...
void System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp)
{
    ...
    mTrackedKeyPointsUn = mpTracker->mCurrentFrame.mvKeysUn;
    current_position_ = Tcw;
    camera_intrinsics_ = mpTracker->mK;
}
...
//------------------------------------------DenseMap-------------------------------------
std::map<MapPoint*,cv::Vec3f> System::GetAllDepthMapPoints() {
    return mpMap->GetAllDepthMapPoints();
}
//------------------------------------------DenseMap-------------------------------------
    //For object tracking------------

cv::Mat System::GetCameraIntrinsics() {
