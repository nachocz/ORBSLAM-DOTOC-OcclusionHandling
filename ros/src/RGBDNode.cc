...
//Couldnt add this to RGBNode.h since there was a conflict and it required static member variables
...
int main(int argc, char **argv)
{
  ...
  pcl::visualization::PCLVisualizer::Ptr newViewer(new pcl::visualization::PCLVisualizer("3D Viewer"));
...
{
  myfile.open("/home/pc-campero2/DatosExperimentos/datos.txt", std::ios_base::app);
...
void RGBDNode::initializeParameters()
{
  ...
  cv::FileStorage fSettings(getDOTsettingsFileNameParam(), cv::FileStorage::READ);
  ...
  voxel_resolution = fSettings["VoxelResolution"]; // 0.01 good for tracking large movements //for a good computation/performance optimization: 0.01f // 0.007
  seed_resolution = fSettings["SeedResolution"];   // 0.04 good for tracking large movements //for a good computation/performance optimization: 0.038f // 0.02

  cout << endl
       << "VCCS parameters:" << endl;
  ...
  cout << endl
       << "LCCP parameters:" << endl;

  concavity_tolerance_threshold = static_cast<float>(fSettings["ConcavityToleranceThreshold"]);
  ...
  cout << "- Concavity tolerance threshold: " << concavity_tolerance_threshold << endl;
  cout << "- Smoothness threshold: " << smoothness_threshold << endl;
  cout << "- Min number of supervoxels in LCCP segment: " << min_segment_size << endl;
  ...
  cout << endl
       << "Deformable Object Tracking parameters:" << endl;
  ...
  segment_manager.max_segment_manager_history_size_ = fSettings["SegmentHistorySize"];
  ...
  myfile << "%% frame_count | Total nºSV | nºSV in object | Residual_0 | Residual_f | Delta_residual | Solver_time | Solver_threads | SLAM time | SV segmentation time | DOT time | Optimization time | fps | general time" << endl;
}
...
void RGBDNode::ImageCallback(const sensor_msgs::ImageConstPtr &msgRGB, const sensor_msgs::ImageConstPtr &msgD)
{
  ...
  viewer->addText(number_of_frames.str(), 10, 300, 20, 0, 0, 0, "number_of_frames text");
  myfile << frame_count << ", ";
  ...
  long int totalMilliseconds_slam_time, totalMilliseconds_sv_segmentation_time, totalMilliseconds_DOT_time, totalMilliseconds_Optimization_time, totalMilliseconds_general_time;
  ...
  if (activate_SLAM)
  {
    ...
    auto elapsed_slam_time = std::chrono::duration_cast<std::chrono::milliseconds>(end_slam_time - start_slam_time);
    ...
    const map<ORB_SLAM2::MapPoint *, cv::Vec3f> &vpMPs = orb_slam_->GetAllDepthMapPoints();
  ...
  //Filtering
  // PointCloudTSuperVoxel::Ptr temp_cloud_rgba_fully_sampled = getPointcloudFromORBSlamMapDataStruct(vpMPs);
  // PointCloudTSuperVoxel::Ptr temp_cloud_rgba(new PointCloudTSuperVoxel);
  // gridSampleApprox(temp_cloud_rgba_fully_sampled, *temp_cloud_rgba, downsampling_grid_size_);

  if (mObjectTrackingState == NO_OBJECT)
  {
    pcl::visualization::PointCloudColorHandlerRGBField<PointTSuperVoxel> rgb(temp_cloud_rgba);
    viewer->addPointCloud<PointTSuperVoxel>(temp_cloud_rgba, rgb, "ColorCloud");
    viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 3, "ColorCloud");
  ...
  pcl::SupervoxelClustering<PointTSuperVoxel> supervoxel_cluster = MakeSuperVoxelCluster(temp_cloud_rgba);
  ...
  auto elapsed_sv_segmentation_time = std::chrono::duration_cast<std::chrono::milliseconds>(end_sv_segmentation_time - start_sv_segmentation_time);
  totalMilliseconds_sv_segmentation_time = elapsed_sv_segmentation_time.count();
  std::ostringstream sv_segmentation_time;
  sv_segmentation_time << "SV time: " << totalMilliseconds_sv_segmentation_time << " ms";
  //viewer->addText(sv_segmentation_time.str(), 10, 80, 20, 0, 0, 0, "sv time text");
  ...
  if (mObjectTrackingState == NO_OBJECT)
  {
    ...
    //LCCP SEGMENTATION
    pcl::LCCPSegmentation<PointT> lccp_segmentation = doLCCPSegmentation(supervoxel_cluster, supervoxel_cluster_map);
    ...
    if (new_clicked_point == true)
    {
      ...
      std::cout << "[INFO] Point coordinate ( " << x_clicking << ", " << y_clicking << ", " << z_clicking << ")" << std::endl;
      std::cout << "[INFO] Point id ( " << clicked_point_id << ")" << std::endl;
      new_clicked_point = false;

      std::shared_ptr<DEF_OBJ_TRACK::Segment> closest_object = getClosestObject(lccp_segmentation, x_clicking, y_clicking, z_clicking, supervoxel_cluster, supervoxel_cluster_map);

      std::map<uint32_t, std::shared_ptr<DEF_OBJ_TRACK::Segment>> segment_list_now;
  ...
  else if (mObjectTrackingState == NEW_OBJECT_SELECTED)
  {
    ...
    trackObject(segment_manager.segment_history_[1][1], supervoxel_cluster, supervoxel_cluster_map);

    auto end_DOT_time = std::chrono::system_clock::now();
    auto elapsed_DOT_time = std::chrono::duration_cast<std::chrono::milliseconds>(end_DOT_time - start_DOT_time);
    ...
    if ((mObjectTrackingState != NO_OBJECT))
    {
      ...
      //Optimization_time << "Opt. time: " << totalMilliseconds_Optimization_time << " ms";
      //viewer->addText(Optimization_time.str(), 10, 40, 20, 0, 0, 0, "Opt. time text");

      mLastProcessedState = mObjectTrackingState;

      std::ostringstream FPS_total_process;
      fps_measurement = 1.0f / ((totalMilliseconds_slam_time + totalMilliseconds_sv_segmentation_time + totalMilliseconds_DOT_time + totalMilliseconds_Optimization_time) * 0.001f);
      FPS_total_process << "fps: " << fps_measurement;
      viewer->addText(FPS_total_process.str(), 10, 120, 20, 0, 0, 0, "fps text");

      //Normal histogram visualization

      // std::vector<double> data_vec;

      // for (int i = 1; i <= segment_manager.segment_list_now_[1]->number_of_sv_in_segment_; i++)
      // {
      //   pcl::PointNormal NormalVector = segment_manager.segment_list_now_[1]->segments_normals_[i];
      ...
      if (save_maps_to_file)
      {
        ...
        sprintf(frame_filename_cloud, "/home/pc-campero2/SavedClouds/%06d.txt", frame_count);
        pcl::io::savePCDFileASCII(frame_filename_cloud, *(segment_manager.segment_list_now_[1]->selected_cloud_with_labeled_sv_));
      }
    }
  }

  auto end_general_time = std::chrono::system_clock::now();
  auto elapsed_general_time = std::chrono::duration_cast<std::chrono::milliseconds>(end_general_time - start_general_time);
  totalMilliseconds_general_time = elapsed_general_time.count();

  //std::ostringstream general_time;
  //general_time << "Time: " << totalMilliseconds_general_time * 0.001f << " s";
  //viewer->addText(general_time.str(), 10, 20, 20, 0, 0, 0, "general_time text");
...
PointCloudTSuperVoxel::Ptr RGBDNode::getPointcloudFromORBSlamMapDataStruct(const std::map<ORB_SLAM2::MapPoint *, cv::Vec3f> &depthmap_points)
{
  ...
  for (std::map<ORB_SLAM2::MapPoint *, cv::Vec3f>::const_iterator it = depthmap_points.begin(); it != depthmap_points.end(); ++it)
  {
    ...
    int rgb = ((int)color_dense[0]) << 16 | ((int)color_dense[1]) << 8 | ((int)color_dense[2]);
...
PointCloudTSuperVoxel::Ptr RGBDNode::PointCloudCreation(const cv::Mat dmcImColor, const cv::Mat dmcImDepth)
{
  ...
  {                                                      //j< siempre "<" no "<="
    for (int i = 0; i < ((imDepth.rows) - 0); i = i + 2) //10)
    {
      ...
      if ((d >= 0.15) && (d <= 0.7)) //Setting the close/far distance thresholds
      {
        PointTSuperVoxel point_in_temp_cloud;
        cv::Mat color_pixel_coord = cv::Mat::zeros(4, 1, CV_32F);

        point_in_temp_cloud.x = d * (j - cameraIntrinsics.at<float>(0, 2)) / cameraIntrinsics.at<float>(0, 0); //d * (j - cameraIntrinsics.at<float>(0, 2)) / cameraIntrinsics.at<float>(0, 0); //
        point_in_temp_cloud.y = d * (i - cameraIntrinsics.at<float>(1, 2)) / cameraIntrinsics.at<float>(1, 1); //d * (i - cameraIntrinsics.at<float>(1, 2)) / cameraIntrinsics.at<float>(1, 1);    //
...
pcl::SupervoxelClustering<PointTSuperVoxel> RGBDNode::MakeSuperVoxelCluster(PointCloudTSuperVoxel::Ptr cloud)
{

  pcl::SupervoxelClustering<PointTSuperVoxel> super(voxel_resolution, seed_resolution);
...
pcl::LCCPSegmentation<PointT> RGBDNode::doLCCPSegmentation(pcl::SupervoxelClustering<PointTSuperVoxel> supervoxel_cluster,
                                                           std::map<uint32_t, pcl::Supervoxel<PointTSuperVoxel>::Ptr> supervoxel_cluster_map)
{
  ...
  pcl::PointCloud<pcl::PointNormal>::Ptr sv_centroid_normal_cloud = pcl::SupervoxelClustering<PointT>::makeSupervoxelNormalCloud(supervoxel_cluster_map);
  ...
  lccp.setSmoothnessCheck(true, supervoxel_cluster.getVoxelResolution(), supervoxel_cluster.getSeedResolution(), 0.1f);
  lccp.setKFactor(0);
  ...
  pcl::PointCloud<pcl::PointXYZL>::Ptr sv_labeled_cloud = supervoxel_cluster.getLabeledCloud();
  ...
  if (VisualizeLCCPConnections_)
  {
    ...
    for (VertexIterator itr = vertex_iterator_range.first; itr != vertex_iterator_range.second; ++itr)
    {
      ...
      for (AdjacencyIterator itr_neighbor = neighbors.first; itr_neighbor != neighbors.second; ++itr_neighbor)
      {
        ...
        pcl::Supervoxel<PointT>::Ptr supervoxel = supervoxel_cluster_map.at(sv_label);
        pcl::PointXYZRGBA vert_curr = supervoxel->centroid_;

        const uint32_t sv_neighbor_label = sv_adjacency_list[*itr_neighbor];
        pcl::Supervoxel<PointT>::Ptr supervoxel_neigh = supervoxel_cluster_map.at(sv_neighbor_label);
  ...
  if (VisualizeLCCPSegments_)
  {
    ...
    viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 1, "maincloud");
    viewer->updatePointCloud((VisualizeSupervoxels_) ? sv_labeled_cloud : lccp_labeled_cloud, "maincloud");
  }

  // /// Show Normals
  // if (show_normals)
  // {
  //  viewer->addPointCloudNormals<pcl::PointNormal>(sv_centroid_normal_cloud, 1, 1.25, "normals");
  // }
  //viewer->spinOnce(100);
  return lccp;
}

std::shared_ptr<DEF_OBJ_TRACK::Segment> RGBDNode::getClosestObject(const pcl::LCCPSegmentation<PointT> &lccp, float xClicked, float yClicked, float zClicked,
                                                                   ...
                                                                   const std::map<uint32_t, pcl::Supervoxel<PointTSuperVoxel>::Ptr> &supervoxel_cluster_map)
{
  ...
  cout << clickedCentroid.x << " " << clickedCentroid.y << " " << clickedCentroid.z << endl;
  ...
  for (std::multimap<uint32_t, uint32_t>::iterator label_itr = supervoxel_adjacency.begin(); label_itr != supervoxel_adjacency.end();)
  {
    ...
    float clickeddistnace = pcl::geometry::distance(clickedCentroid, supervoxelCentroid);
  ...
  for (std::map<uint32_t, std::set<uint32_t>>::iterator label_LCCP_iterator = segment_supervoxel_map_arg.begin(); label_LCCP_iterator != segment_supervoxel_map_arg.end();)
  {
    ...
    label_LCCP_iterator = segment_supervoxel_map_arg.upper_bound(label_LCCP_iterator->first);
  }

  cout << "closestmObjectTrackingState supervoxel label: " << closest_supervoxel_label << endl;
  cout << "closest segment label: " << closest_segment_label << endl;

  std::set<uint32_t> SVs_in_closest_segment = segment_supervoxel_map_arg[closest_segment_label];
  std::set<uint32_t>::iterator itr_closest_segment;

  pcl::PointCloud<PointTSuperVoxel>::Ptr selected_cloud_with_labeled_sv_(new pcl::PointCloud<PointTSuperVoxel>);
  std::shared_ptr<DEF_OBJ_TRACK::Segment> initial_object = std::make_shared<DEF_OBJ_TRACK::Segment>(selected_cloud_with_labeled_sv_, neighbouringRadius, explorationRadius, DEF_OBJ_TRACK::Segment::SegmentType::TARGET_OBJECT);

  for (itr_closest_segment = SVs_in_closest_segment.begin(); itr_closest_segment != SVs_in_closest_segment.end(); ++itr_closest_segment) //iterando en los supervoxels de pertenecen al segmento en le que se ha clickado
  {
    pcl::PointXYZRGBA SVcentroid;
    pcl::PointNormal newNormal;

    uint32_t supervoxel_label = *itr_closest_segment;
    pcl::Supervoxel<PointTSuperVoxel>::Ptr supervoxel = supervoxel_cluster_map.at(supervoxel_label);
    ...
    initial_object->segments_sv_map_.insert(std::pair<uint32_t, pcl::Supervoxel<PointTSuperVoxel>::Ptr>(initial_object->label_of_sv_, supervoxel));
    initial_object->segments_colors_RGB_.insert(std::pair<uint32_t, pcl::PointXYZRGBA>(initial_object->label_of_sv_, SVcolorRGB));
    initial_object->segments_normals_.insert(std::pair<uint32_t, pcl::PointNormal>(initial_object->label_of_sv_, newNormal));

    *initial_object->selected_cloud_with_labeled_sv_ += *supervoxel->voxels_; //adding the pointcloud of the accepted voxel to the objects pointcloud
  }
  ...
  initial_object->segmentType_ = DEF_OBJ_TRACK::Segment::SegmentType::TARGET_OBJECT;

  return initial_object;
}

void RGBDNode::trackObject(std::shared_ptr<DEF_OBJ_TRACK::Segment> previousObject, pcl::SupervoxelClustering<PointTSuperVoxel> &supervoxel_cluster,
                           const std::map<uint32_t, pcl::Supervoxel<PointTSuperVoxel>::Ptr> &supervoxel_cluster_map)
{
  ...
  pcl::PointCloud<PointTSuperVoxel>::Ptr selected_cloud_with_labeled_sv(new pcl::PointCloud<PointTSuperVoxel>);
  ...
  for (std::multimap<uint32_t, uint32_t>::iterator label_itr = supervoxel_adjacency.begin(); label_itr != supervoxel_adjacency.end();)
  {
    ...
    supervoxel->getCentroidPoint(SVcentroid);
    pcl::PointXYZRGBA SVcentroid_on_objects_ref = previousObject->computeCentroidCoordInObjectsReference(SVcentroid);
    ...
    if (previousObject->isItInsideAugmentedOBB(SVcentroid_on_objects_ref))
    {
      ...
      for (int i = 1; ((i <= (segment_manager.segment_manager_history_size_)) && not_belonging_yet); i++)
      {
        if (segment_manager.doesItBelongToSegment(SVcolorRGB, newNormal, segment_manager.segment_history_[i][1], viewer))
        {

          NewObject->label_of_sv_++;
          NewObject->segments_sv_map_.insert(std::pair<uint32_t, pcl::Supervoxel<PointTSuperVoxel>::Ptr>(NewObject->label_of_sv_, supervoxel));
          NewObject->segments_colors_RGB_.insert(std::pair<uint32_t, pcl::PointXYZRGBA>(NewObject->label_of_sv_, SVcolorRGB));
          NewObject->segments_normals_.insert(std::pair<uint32_t, pcl::PointNormal>(NewObject->label_of_sv_, newNormal));
          //pcl::PointCloud<PointTSuperVoxel>::Ptr voxelsPointcloud = supervoxel->voxels_;
...
void RGBDNode::addSupervoxelConnectionsToViewer(PointTSuperVoxel &supervoxel_center,
                                                ...
                                                std::string supervoxel_name)
{
  ...
  for (pcl::PointCloud<PointTSuperVoxel>::iterator adjacent_itr = adjacent_supervoxel_centers.begin(); adjacent_itr != adjacent_supervoxel_centers.end(); ++adjacent_itr)
  {
...
pcl::PointXYZRGBA RGBDNode::computeColorRGB(const pcl::PointXYZRGBA &SVcentroid)
{
  pcl::PointXYZRGBA SVcolorRGB;
...
void RGBDNode::gridSampleApprox(const PointCloudTSuperVoxel::Ptr &cloud, PointCloudTSuperVoxel &result, double leaf_size)
{
