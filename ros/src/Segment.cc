...
namespace DEF_OBJ_TRACK
{
    ...
    Segment::Segment(pcl::PointCloud<PointTSuperVoxel>::Ptr selected_cloud_with_labeled_sv, float neighbouring_radius, float exploration_radius, Segment::SegmentType typeOfSegment) : selected_cloud_with_labeled_sv_(selected_cloud_with_labeled_sv), neighbouring_radius_(neighbouring_radius),
                                                                                                                                                                                       exploration_radius_(exploration_radius), label_of_sv_(0), segmentType_(typeOfSegment)
    {
    ...
    void Segment::computeFeatureExtraction()
    {
        if (segmentType_ == TARGET_OBJECT)
        {
            ...
            feature_extractor_.getOBB(min_point_OBB_, max_point_OBB_, position_OBB_, rotational_matrix_OBB_);
            feature_extractor_.getEigenValues(major_value_, middle_value_, minor_value_);
            feature_extractor_.getEigenVectors(major_vector_, middle_vector_, minor_vector_);
    ...
    pcl::PointXYZRGBA Segment::computeCentroidCoordInObjectsReference(pcl::PointXYZRGBA SVcentroid)
    {
        pcl::PointXYZRGBA SVcentroid_on_objects_ref;

        SVcentroid_on_objects_ref.x = (SVcentroid.x - mass_center_(0)) * major_vector_(0) +
                                      (SVcentroid.y - mass_center_(1)) * major_vector_(1) +
                                      (SVcentroid.z - mass_center_(2)) * major_vector_(2);
        SVcentroid_on_objects_ref.y = (SVcentroid.x - mass_center_(0)) * middle_vector_(0) +
                                      (SVcentroid.y - mass_center_(1)) * middle_vector_(1) +
                                      (SVcentroid.z - mass_center_(2)) * middle_vector_(2);
        SVcentroid_on_objects_ref.z = (SVcentroid.x - mass_center_(0)) * minor_vector_(0) +
    ...
    {
        float avgModule = sqrt(((average_normal_.normal_x) * (average_normal_.normal_x)) + ((average_normal_.normal_y) * (average_normal_.normal_y)) + ((average_normal_.normal_z) * (average_normal_.normal_z)));
    ...
    void Segment::computeAngularError()
    {
        ...
        image_plane_coord = camera_intrinsics_extended_ * camera_position_ * centroid_position; //reprojectiong 3D points from RGB camera center ref. to the RGB camera projection plane
    ...
    void Segment::computePositionError()
    {
        ...
        //     {
        //         t_objetivo(iAffine, jAffine) = invPosition.at<float>(iAffine, jAffine);
        //     }
        // }
        ...
        average_normal_global_reference_.normal_x = average_normal_.normal_x - invPosition.at<float>(0, 3);
        average_normal_global_reference_.normal_y = average_normal_.normal_y - invPosition.at<float>(1, 3);
        average_normal_global_reference_.normal_z = average_normal_.normal_z - invPosition.at<float>(2, 3);
    ...
    //OBB
    bool Segment::isItInsideAugmentedOBB(const pcl::PointXYZRGBA &SVcentroid_on_objects_ref)
    {
        if ((SVcentroid_on_objects_ref.x >= (min_point_OBB_.x - tol1X_)) && (SVcentroid_on_objects_ref.x <= (max_point_OBB_.x + tol1X_)) && (SVcentroid_on_objects_ref.y >= (min_point_OBB_.y - tol1Y_)) && (SVcentroid_on_objects_ref.y <= (max_point_OBB_.y + tol1Y_)) && (SVcentroid_on_objects_ref.z >= (min_point_OBB_.z - tol1Z_)) && (SVcentroid_on_objects_ref.z <= (max_point_OBB_.z + tol1Z_)))
        {
            return true;
        }
        else
    ...
    void Segment::computeLargestDistanceToCamera(pcl::PointCloud<pcl::PointNormal>::Ptr visualization_normal_cloud)
    {
    ...
    void Segment::computeInterestfrustum(pcl::PointCloud<pcl::PointNormal>::Ptr visualization_normal_cloud) //Not working properly, objects frame of reference variates too much
    {

        //NOT WORTH IT.........
        // //computing max and min obb points on camera refrence to check which face of the OBB is closer to the camera
        // //////////////////////MAX////////////////////////
        ...
        //     max_point_OBB_.y,
        //     max_point_OBB_.z;
        // Eigen::Vector3f max_OBB_world_ref = mass_center_ + rotational_matrix_OBB_ * max_OBB_object_ref;
        ...
        // max_point_OBB_camera_ref = camera_position_ * max_point_OBB_world_ref_mat; //reprojectiong 3D points from RGB camera center ref. to the RGB camera projection plane
        ...
        //     min_point_OBB_.y,
        //     min_point_OBB_.z;
        // Eigen::Vector3f min_OBB_world_ref = mass_center_ + rotational_matrix_OBB_ * min_OBB_object_ref;
        ...
        // min_point_OBB_camera_ref = camera_position_ * min_point_OBB_world_ref_mat; //reprojectiong 3D points from RGB camera center ref. to the RGB camera projection plane

        // //computing frustum corners on objects reference

        // Eigen::Matrix<float, 3, 4> corners;
        // cout << "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%" << endl;
        // cout << "max_point_OBB_camera_ref(2): " << max_point_OBB_camera_ref(2) << endl;
        // cout << "min_point_OBB_camera_ref(2): " << min_point_OBB_camera_ref(2) << endl;
        ...
        //         max_point_OBB_.z + tol1Z_, max_point_OBB_.z + tol1Z_, max_point_OBB_.z + tol1Z_, max_point_OBB_.z + tol1Z_;
        // }
        // else
        // {
        ...
        for (uint32_t i = 0; i < 4; i++) //watchout, eigen begins in 0 index
        {
            ...
            Eigen::Vector3f point_position = mass_center_ + rotational_matrix_OBB_ * shift;
            ...
            visualizationVector.normal_x = point_position(0) - Twc_depth_.at<float>(0, 3);
            visualizationVector.normal_y = point_position(1) - Twc_depth_.at<float>(1, 3);
            visualizationVector.normal_z = point_position(2) - Twc_depth_.at<float>(2, 3);
    ...
    {
        cv::Mat camera_intrinsics_extended = cv::Mat::eye(4, 4, camera_position_.type());
        camera_intrinsics_.copyTo(camera_intrinsics_extended.rowRange(0, 3).colRange(0, 3));
                                                    ...
                                                    std::string supervoxel_name, pcl::visualization::PCLVisualizer::Ptr viewer)
    {
        ...
        for (pcl::PointCloud<pcl::PointXYZ>::iterator adjacent_itr = adjacent_supervoxel_centers.begin(); adjacent_itr != adjacent_supervoxel_centers.end(); ++adjacent_itr)
        {
    ...
    void Segment::visualizeObjectsReprojectionOnCamera(std::shared_ptr<DEF_OBJ_TRACK::Segment> occlusions, std::shared_ptr<DEF_OBJ_TRACK::Segment> hardOcclusions)
    {
        ...
        cv::circle(atom_image, cv::Point(camera_to_object_frustum_image_coord_[1].at<float>(0, 0), camera_to_object_frustum_image_coord_[1].at<float>(1, 0)), radius, blue);
        cv::circle(atom_image, cv::Point(camera_to_object_frustum_image_coord_[2].at<float>(0, 0), camera_to_object_frustum_image_coord_[2].at<float>(1, 0)), radius, blue);
        cv::circle(atom_image, cv::Point(camera_to_object_frustum_image_coord_[3].at<float>(0, 0), camera_to_object_frustum_image_coord_[3].at<float>(1, 0)), radius, blue);
        cv::circle(atom_image, cv::Point(camera_to_object_frustum_image_coord_[4].at<float>(0, 0), camera_to_object_frustum_image_coord_[4].at<float>(1, 0)), radius, blue);
        ...
        for (int i = 1; i <= occlusions->label_of_sv_; i++)
        {
            ...
            cv::circle(atom_image, cv::Point(image_plane_coord.at<float>(0, 0), image_plane_coord.at<float>(1, 0)), radius, yellow);
        }
        ...
        for (int i = 1; i <= hardOcclusions->label_of_sv_; i++)
        {
            ...
            cv::circle(atom_image, cv::Point(image_plane_coord.at<float>(0, 0), image_plane_coord.at<float>(1, 0)), radius_15, red);
        }
    ...
    void Segment::computeOctreeAdjacencyAndDegree(pcl::visualization::PCLVisualizer::Ptr viewer)
    {
        ...
        for (std::size_t i = 0; i < label_of_sv_; ++i) //index for the cloud is one less than for the maps
        {
        ...
        {
            if (Segments_sv_AdjacencyOctree_->radiusSearch(temp_cloud->points[j], neighbouring_radius_, pointIdxRadiusSearch, pointRadiusSquaredDistance) > 0)
            {
                pcl::PointCloud<pcl::PointXYZ> adjacent_supervoxel_centers;
                ...
                {
                    adjacent_supervoxel_centers.push_back(temp_cloud->points[pointIdxRadiusSearch[i]]);
                }
    ...
    void Segment::VisualizeCloudsAndBoundingBoxes(pcl::visualization::PCLVisualizer::Ptr viewer)
    {
        ...
        // viewer->addCube( min_point_AABB_.x,  max_point_AABB_.x,  min_point_AABB_.y,  max_point_AABB_.y,  min_point_AABB_.z,  max_point_AABB_.z, 1.0, 1.0, 0.0, "AABB");
        ...
        // viewer->addCube(position, quat,  max_point_OBB_.x -  min_point_OBB_.x,  max_point_OBB_.y -  min_point_OBB_.y,  max_point_OBB_.z -  min_point_OBB_.z, "OBB");
        // viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_REPRESENTATION, pcl::visualization::PCL_VISUALIZER_REPRESENTATION_WIREFRAME, "OBB");

        viewer->addCube(position, quat, max_point_OBB_.x - min_point_OBB_.x + 2 * tol1X_, max_point_OBB_.y - min_point_OBB_.y + 2 * tol1Y_, max_point_OBB_.z - min_point_OBB_.z + 2 * tol1Z_, "augmentedOBB");
        viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_REPRESENTATION, pcl::visualization::PCL_VISUALIZER_REPRESENTATION_WIREFRAME, "augmentedOBB");

        // viewer->addCube(position, quat,  max_point_OBB_.x -  min_point_OBB_.x + 2 *  tolX_,  max_point_OBB_.y -  min_point_OBB_.y + 2 *  tolY_,  max_point_OBB_.z -  min_point_OBB_.z + 2 *  tolZ_, "reducedOBB");
        // viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_REPRESENTATION, pcl::visualization::PCL_VISUALIZER_REPRESENTATION_WIREFRAME, "reducedOBB");
        ...
        if (visualize_rgba_pointcloud)
        {
            ...
            viewer->addPointCloud<PointTSuperVoxel>(selected_cloud_with_labeled_sv_, "sample cloud");
            viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 4, "sample cloud");

            // PointCloudTSuperVoxel::Ptr voxel_centroid_cloud = supervoxel_cluster.getVoxelCentroidCloud();
            // viewer->addPointCloud(voxel_centroid_cloud, "voxel centroids");
            // viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 50.0, "voxel centroids");
            // viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_OPACITY, 1.0, "voxel centroids");

            // PointLCloudTSuperVoxel::Ptr labeled_voxel_cloud = supervoxel_cluster.getLabeledVoxelCloud();
        ...
        if (show_normals)
        {
            pcl::PointCloud<pcl::PointNormal>::Ptr sv_normal_cloud(new pcl::PointCloud<pcl::PointNormal>);
            ...
            viewer->addPointCloudNormals<PointNTSuperVoxel>(sv_normal_cloud, 1, 0.05f, "supervoxel_normals");
            viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, 1.0, 0.0, 0.0, "supervoxel_normals");
            viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_LINE_WIDTH, 1, "supervoxel_normals");
        }
    }
    ...
    void Segment::OcclusionOptimizationVisualChecking(std::shared_ptr<DEF_OBJ_TRACK::Segment> NewObject, Eigen::Affine3f t_objetivo,pcl::visualization::PCLVisualizer::Ptr viewer)
    {
        ...
        int num_occlusions_ = static_cast<int>(number_of_sv_in_segment_); //remember, I was using index from 1 to inf, here from 0
        double *occlusions_ = new double[6 * number_of_sv_in_segment_];

        for (int i_checking = 0; i_checking < num_occlusions_; ++i_checking)
        {
            pcl::PointNormal sv_normal = segments_normals_[i_checking + 1]; //i_checkingndex i_checkingn maps start at 1 for me (0 preserved for special cases)
            ...
            occlusions_[i_checking * 6 + 3] = static_cast<double>(sv_normal.x + sv_normal.normal_x);
            occlusions_[i_checking * 6 + 4] = static_cast<double>(sv_normal.y + sv_normal.normal_y);
            occlusions_[i_checking * 6 + 5] = static_cast<double>(sv_normal.z + sv_normal.normal_z);
            ...
            Eigen::Matrix<double, 3, 1>
                V_vector; //3rd basis vector
            V_vector = (normal_vector.cross(U_vector)).normalized();

            Eigen::Matrix<double, 3, 1> W_vector; //Comprobation vector, it must be = normal_vector
            ...
            OBBCorners_and_sv_vector << double(occlusion_x), double(OBBCorner_1_x), double(OBBCorner_2_x), double(OBBCorner_3_x), double(OBBCorner_4_x),
                double(occlusion_y), double(OBBCorner_1_y), double(OBBCorner_2_y), double(OBBCorner_3_y), double(OBBCorner_4_y),
                double(occlusion_z), double(OBBCorner_1_z), double(OBBCorner_2_z), double(OBBCorner_3_z), double(OBBCorner_4_z);
            ...
            occlusion_vector << double(occlusion_x),
                double(occlusion_y),
                double(occlusion_z),
                double(1.0);
            ...
            {
                cv::circle(atom_image, cv::Point(float(OBBcorn_and_sv_plane(0, 0)), float(OBBcorn_and_sv_plane(1, 0))), radius_15, red);
            }
            else
            {
                cv::circle(atom_image, cv::Point(float(OBBcorn_and_sv_plane(0, 0)), float(OBBcorn_and_sv_plane(1, 0))), radius_15, yellow);
            }
            ...
            cv::circle(atom_image, cv::Point(float(OBBcorn_and_sv_plane(0, 1)), float(OBBcorn_and_sv_plane(1, 1))), radius, blue);
            cv::circle(atom_image, cv::Point(float(OBBcorn_and_sv_plane(0, 2)), float(OBBcorn_and_sv_plane(1, 2))), radius, blue);
            cv::circle(atom_image, cv::Point(float(OBBcorn_and_sv_plane(0, 3)), float(OBBcorn_and_sv_plane(1, 3))), radius, blue);
            cv::circle(atom_image, cv::Point(float(OBBcorn_and_sv_plane(0, 4)), float(OBBcorn_and_sv_plane(1, 4))), radius, blue);
            ...
            if (it_is_in_frustum && occlusion_between_cam_obj)
            {
                weight = ((-weight_scale / cone_radius) * (sqrt(testx * testx + testy * testy))) + (weight_scale); // / normalizing_distance;
            }
            else
            {
                weight = double(0.0);
        ...
        for (uint32_t i = 1; i < 5; i++) //watchout, eigen begins in 0 index
        {
            ...
            visualizationVector.normal_x = NewObject->camera_to_object_frustum_[i].x - t_objetivo(0, 3);
