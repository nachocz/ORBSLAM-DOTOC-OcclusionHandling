...
public:
  enum SegmentType {
    ...
    INTERACTING_ELEMENT =
        1, // When it is inside the objects interacting zone (i.e. OBB)
    OCCLUDING_ELEMENT = 2, // When it's depth (camera reference) is smaller than
                           // the furthest point of the target object
    HARD_OCCLUDING_ELEMENT =
        3, // When is in the frustum defined by the OBB and the camera centre
  };
...
public:
  Segment();
  ~Segment();
  Segment(pcl::PointCloud<PointTSuperVoxel>::Ptr selected_cloud_with_labeled_sv,
  ...
  // Utilities
  pcl::PointXYZRGBA
  computeCentroidCoordInObjectsReference(pcl::PointXYZRGBA SVcentroid);
  void computeInterestfrustum(
      pcl::PointCloud<pcl::PointNormal>::Ptr visualization_normal_cloud);

  bool
  isItInsideAugmentedOBB(const pcl::PointXYZRGBA &SVcentroid_on_objects_ref);
  ...
  void computeOctreeAdjacencyAndDegree(
      pcl::visualization::PCLVisualizer::Ptr viewer);

  // adjacency visualization

  void addSupervoxelConnectionsToViewer2(
  ...
  void computeLargestDistanceToCamera(
      pcl::PointCloud<pcl::PointNormal>::Ptr visualization_normal_cloud);
  bool isItInOcclusionsDepthRange(pcl::PointXYZRGBA sv_centroid);
  bool isItInCameraToObjectFrustum(pcl::PointXYZRGBA sv_centroid);
  bool isItInPerceptionSphere(pcl::PointXYZRGBA sv_centroid,
                              const double &sphere_radius);
  void visualizeObjectsReprojectionOnCamera(
      std::shared_ptr<DEF_OBJ_TRACK::Segment> occlusions,
      std::shared_ptr<DEF_OBJ_TRACK::Segment> hardOcclusions);

  // General visualization
  void
  OcclusionVisualChecking(std::shared_ptr<DEF_OBJ_TRACK::Segment> NewObject,
                          Eigen::Affine3f t_objetivo,
                          pcl::visualization::PCLVisualizer::Ptr viewer);
  void VisualizeCloudsAndBoundingBoxes(
      pcl::visualization::PCLVisualizer::Ptr viewer);
  ...
  normalsToSphereIntersectionPoints(bool visualize_rays,
      ...
      const double &sphere_radius);
  std::map<uint32_t, Eigen::Matrix<float, 3, 1>>
  centroidsToOcclussorRays(pcl::visualization::PCLVisualizer::Ptr viewer,
                           const double &sphere_radius,
  ...
  computePerpendicularVector(Eigen::Matrix<float, 1, 3> v_input);

public:
  // Main structure of a Segment

  // Max value of sv_label_ = nº of SVs in the segment. Use sv_label_ to access
  // each SV
  uint32_t number_of_sv_in_segment_;
  uint32_t label_of_sv_;

  // Pointcloud of the Segment, don't use its labels as a reference since they
  // wont correspond the ones of the calss Segment
  pcl::PointCloud<pcl::PointXYZL>::Ptr
      segments_voxel_labeled_cloud_; // previously
                                     // selected_cloud_with_labeled_sv_
  pcl::PointCloud<PointTSuperVoxel>::Ptr selected_cloud_with_labeled_sv_;

  // Maps that storea the  Segment's Supervoxels. RGB and normal information is
  // already in each sv, but having separated maps is more clear
  ...
  std::map<uint32_t, pcl::PointNormal>
      segments_normals_; // Each normalPoint also stores the SV's centroid
                         // information, starting and ending point of the normal
                         // vector
  std::map<uint32_t, pcl::PointNormal>
      segment_normals_on_world_reference_; // point= world referenc origin,
                                           // normal= vector in world reference

  enum SegmentState {
    LOST = 0,    // Nothing can be done, need to go back to lccp segmentation of
                 // the scene
    VISIBLE = 1, // When it is fully or partially visible
    OCCLUDED =
        2, // When it is completely occluded but re-location tasks are running
  };
...
public:
  ...
  cv::Mat camera_position_, camera_intrinsics_,
      camera_intrinsics_extended_; // T world->camera
...
public:
  ...
  std::map<uint32_t, pcl::PointXYZ>
      camera_to_object_frustum_; // stores the 4 corners (world referenced) of
                                 // the middle plane of the augmented OBB
